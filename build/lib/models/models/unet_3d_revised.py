...
class hidden_spectral_weighting(nn.Module):
    def __init__(self, chunks, cross_attention_dim, attention_head_dim):
        super().__init__()
        self.weight = nn.init.normal_(nn.Parameter(torch.randn(chunks,1, cross_attention_dim, attention_head_dim//2, attention_head_dim//2))) #self.get_dct_filter(height, width, mapper_x, mapper_y, channel))
        self.dwt = DWT_2D('haar')
        self.idwt = IDWT_2D('haar')
        self.fc = nn.Sequential(
            nn.Linear(attention_head_dim, attention_head_dim//2, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(attention_head_dim//2, attention_head_dim, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, samples):
        ...
        #print(samples.shape, self.weight.shape)
        ll, hl, lh, hh = self.dwt(samples)
        
        weight_samples = torch.einsum('n b c h w, n m c s t -> n b c h w', torch.stack([ll, hl, lh, hh], 0), self.weight)
        weight_samples = self.idwt(weight_samples[0], weight_samples[1], weight_samples[2], weight_samples[3])
        samples = self.fc(samples)
        samples = samples * weight_samples 
        samples = rearrange(samples, '(b f) c h w -> b c f h w', f=f)
        return samples
...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        ##
        self.hidden_spectral_weighting = hidden_spectral_weighting(4, block_out_channels[-1], attention_head_dim[-1])
    ...
    def forward(
        ...
        sample = self.hidden_spectral_weighting(sample)
        #print(sample.shape)
