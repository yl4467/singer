...
class DWT_2D(Module):
    ...
    def get_matrix(self, dtypes):
        ...
        if torch.cuda.is_available():
            self.matrix_low_0 = torch.Tensor(matrix_h_0).half().cuda()
            self.matrix_low_1 = torch.Tensor(matrix_h_1).half().cuda()
            self.matrix_high_0 = torch.Tensor(matrix_g_0).half().cuda()
            self.matrix_high_1 = torch.Tensor(matrix_g_1).half().cuda()
        else:
            self.matrix_low_0 = torch.Tensor(matrix_h_0)
            self.matrix_low_1 = torch.Tensor(matrix_h_1)
            self.matrix_high_0 = torch.Tensor(matrix_g_0)
    ...
    def forward(self, input):
        ...
        self.input_height = input.size()[-2]
        self.input_width = input.size()[-1]
        self.get_matrix(dtypes=input.type)
