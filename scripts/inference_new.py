...
from models.animate.face_animate_revised import FaceAnimatePipeline

from models.datasets.audio_processor import AudioProcessor
from models.datasets.image_processor import ImageProcessor
from models.models.audio_proj import AudioProjModel
from models.models.audio_spectral_embed import MultiSpectralAttentionLayer
from models.models.face_locator import FaceLocator
...
from models.utils.config import filter_non_none
from models.utils.util import tensor_to_video
...
def inference_process(args: argparse.Namespace):
    ...
    audioembedder = MultiSpectralAttentionLayer(config.data.bs, 14, 4, 64, 64)
    ...
    audioembedder.requires_grad_(False)
    ...
    m,u = net.load_state_dict(
        torch.load(
            os.path.join(audio_ckpt_dir),
            map_location="cpu",
        ), strict=False
    )
    assert len(m) == 0 and len(u) == 0, "Fail to load correct checkpoint."
    print("loaded weight from ", os.path.join(audio_ckpt_dir))
    ...
    output_file = os.path.join(config.output, os.path.basename(driving_audio_path)[:-4]+'.mp4')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--config", default="configs/inference/inference.yaml")
    parser.add_argument("--source_image", type=str, required=False,
                        help="source image", default="examples/0001.png")
    parser.add_argument("--driving_audio", type=str, required=False,
                        help="driving audio", default="examples/0001.wav")
    parser.add_argument(
        "--output", type=str, help="output video file name", default="results/")
    parser.add_argument(
